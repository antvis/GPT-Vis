...
| 实体类型             | 描述                                                        | 示例                                                                      |
| -------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------- |
| `metric_name`        | 指标名称，通常是句子的主语，数据分析中的主要指标            | `[销售额](metric_name)`、`[DAU](metric_name)`                             |
| `metric_value`       | 指标值，通常作为宾语，表示具体的数值                        | `[¥150 万](metric_value, origin=1500000)`、`[1,234 人](metric_value)`     |
| `other_metric_value` | 次要指标值，用于辅助指标                                    | `[平均: ¥120](other_metric_value)`                                        |
| `dim_name`           | 维度名称，表达数据分析中可以拆分下钻的维度                  | `[省份](dim_name)`、`[城市](dim_name)`、`[性别](dim_name)`                |
| `dim_value`          | 维度值，通常是句子的主语、宾语或定语，表示具体的维度取值    | `[北京](dim_value)`、`[女性](dim_value)`、`[支付宝](dim_value)`           |
| `time_desc`          | 时间描述，通常在句子开始，限定数据结论的日期范围            | `[2024 年 Q3](time_desc)`、`[本月](time_desc)`、`[2023-01-23](time_desc)` |
| `trend_desc`         | 趋势描述，通常作为宾语，描述指标的变化趋势                  | `[上升](trend_desc)`、`[下降](trend_desc)`、`[平稳](trend_desc)`          |
| `delta_value`        | 绝对变化值，描述对比差值（a-b），表示增减的数值             | `[+1,200](delta_value)`、`[3000](delta_value)`                            |
| `delta_value_pos`    | 正向绝对变化值，用于表示增长的绝对数值（需要进行 abs 处理） | `[3000](delta_value_pos)`（原值为正）                                     |
| `delta_value_neg`    | 负向绝对变化值，用于表示下降的绝对数值（需要进行 abs 处理） | `[500](delta_value_neg)`（原值为 -500）                                   |
| `ratio_value`        | 百分比变化值，描述对比率（(a-b)/b），表示增长率或下降率     | `[+15.3%](ratio_value, origin=0.153)`、`[30%](ratio_value)`               |
| `ratio_value_pos`    | 正向百分比变化值，用于表示增长率（需要进行 abs 处理）       | `[15.3%](ratio_value_pos, origin=0.153)`（原值为正）                      |
| `ratio_value_neg`    | 负向百分比变化值，用于表示下降率（需要进行 abs 处理）       | `[8.2%](ratio_value_neg, origin=0.082)`（原值为 -0.082）                  |
| `contribute_ratio`   | 贡献占比，表示某部分占总体的比例                            | `[45%](contribute_ratio, origin=0.45)`                                    |
| `proportion`         | 比例或占比，表示该指标占总体的比例                          | `[3/5](proportion, origin=0.6)`、`[22%](proportion)`                      |
