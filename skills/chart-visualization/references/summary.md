...
T8 语法是一种类 Markdown 的声明式语言，用于创建带有语义实体标注的数据叙事文本。
...
- **斜体**：`这是*斜体文本*`
- **下划线**：`这是__下划线文本__`
- **链接**：`访问[我们的网站](https://example.com)`

#### 实体标注语法

T8 语法的核心特性是**实体标注**，即用语义标记数据点。
...
```
[销售收入](metric_name)达到[¥150 万](metric_value)。
```
...
```
收入增长了[15.3%](ratio_value, origin=0.153, assessment="positive")。
```
