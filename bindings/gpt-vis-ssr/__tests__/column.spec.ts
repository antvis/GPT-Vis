...
describe('SSR render', () => {
  ...
  it('column-grouped-label', async () => {
    ...
    expect(vis.toBuffer()).toImageEqual('__tests__/snapshot', 'column-grouped-label');
  });
  ...
  it('column-grouped-academy-label', async () => {
    const vis = await render({
      width: 600,
      height: 400,
      type: 'column',
    ...
    expect(vis.toBuffer()).toImageEqual('__tests__/snapshot', 'column-grouped-academy-label');
  });
