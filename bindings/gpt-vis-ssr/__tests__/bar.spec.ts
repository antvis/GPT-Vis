...
describe('SSR render', () => {
  ...
  it('bar-grouped-label', async () => {
    ...
    expect(vis.toBuffer()).toImageEqual('__tests__/snapshot', 'bar-grouped-label');
  });
  ...
  it('bar-stacked-academy-label', async () => {
    const vis = await render({
      theme: 'academy',
      width: 600,
      height: 400,
    ...
    expect(vis.toBuffer()).toImageEqual('__tests__/snapshot', 'bar-stacked-academy-label');
  });
});
