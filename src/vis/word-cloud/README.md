...
词云图（Word Cloud Chart），用于以视觉化方式展示文本数据中词语的频率或重要性。组件会按权重降序展示 Top 50，使用全局分类色板区分不同词语，并通过 Impact 字体、字号、字重和多角度布局表达视觉层级。布局使用可复现的随机序列，相同数据重复渲染时保持稳定。悬停词语时会通过轻微放大和贴近字形的短投影突出当前词，并展示权重或词频、排名和占总权重比例。
...
| 属性                  | 类型                | 是否必填 | 默认值    | 说明                                                |
| --------------------- | ------------------- | -------- | --------- | --------------------------------------------------- |
| type                  | string              | 必填     | -         | 图表类型，值为 "word-cloud"                         |
| data                  | WordCloudDataItem[] | 必填     | -         | 词云图数据                                          |
| data[n].text          | string              | 必填     | -         | 词语文本                                            |
| data[n].value         | number              | 必填     | -         | 词语权重或频率                                      |
